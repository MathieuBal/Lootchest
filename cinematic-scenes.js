// Lootchest — intro cinematic scene data.
// Scene 1 (L'Oubli) uses the REAL produced planches in assets/cinematic/.
// Scenes 2–7 are enriched SVG placeholders, structured with the SAME layer
// model (bg / mid / fg / fx + depth for parallax) so the dev swaps each SVG
// for a real PNG planche later. Texts are the EXACT ones from cinematic.js.

(function () {
  const L = (svg, depth, z) => ({ svg, depth, z });
  // helper: a full-bleed <img> layer pointing at a produced PNG
  const IMG = (src) => `<img src="${src}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;image-rendering:auto" alt="">`;
  // SVG helpers for placeholder scenes
  ...
  const motes = (n, fill, cls) => Array.from({ length: n }, (_, i) =>
    `<circle class="${cls || 'fx-mote'}" style="--d:${(i % 6) * 0.7}s" cx="${5 + (i * 13) % 90}" cy="${20 + (i * 29) % 70}" r="${0.4 + (i % 3) * 0.4}" fill="${fill}"/>`).join('');
  const SVG = (inner) => `<svg viewBox="0 0 100 100" preserveAspectRatio="xMidYMid slice">${inner}</svg>`;

  const SCENES = [
    // ───────── 1 · L'Oubli — REAL planches ─────────
    {
  ...
  window.CINEMATIC_SCENES = SCENES;
})();
